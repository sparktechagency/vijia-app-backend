...
import { AddressType, getaddressFromTheAi, getChatbotPrompt } from './chatbot.constants';
...
const  generateAiResponse = async (user:JwtPayload,prompt?:string) => {
    ...
    if(Object.values(json.data??{}).some(data=>data.length>0)){
        ...
        io.emit('get-chatbot::'+user.id, { message: data.message, sender: 'ai',user: user.id,data: data.data });
        return data
    }
    ...
    if(json?.responseItems?.includes('hotel')){
        const hotelsData = await amaduesHelper.getHotelsList(json.destinationAitaCity,100,5)

        lengthData.hotels = convertHotelIntoHomeItem(hotelsData.data?.slice(0, 10)) as any
    ...
    io.emit('get-chatbot::'+user.id, { message: responsek.message, sender: 'ai',user: user.id ,data:responsek.data});
...
const getMessagesOFChatbot = async (user:JwtPayload,query:Record<string,any>) => {
    const chatbotQuery = new QueryBuilder(Chatbot.find({ user: user.id }), query).paginate().sort()
