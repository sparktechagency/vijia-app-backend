...
const getCityInfo = async (city: string, country: string): Promise<typeof sampleForm> => {
  try {
    const prompt = `
...
const createAiSuggestionTravelDestination = async (user: string,address:{city:string,country:string}) => {
 try {
  ...
  const prompt = `
My user type is ${userDetails?.user_type} and their interested categories are: ${userDetails?.interested_categories?.join(',')}.
